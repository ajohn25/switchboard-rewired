...
create view sms.phone_number_capacity as
  select
    count(distinct case when created_at > now() - interval '36 hours' then to_number else null end) as fresh_commitment_count,
    count(distinct to_number) as full_commitment_count,
    from_number
  from sms.outbound_messages
  group by from_number
  union
  select 0 as fresh_commitment_count, 0 as full_commitment_count, phone_number
...
create function sms.send_message (profile_id uuid, "to" phone_number, body text, media_urls url[], contact_zip_code zip_code default null) returns sms.outbound_messages as $$
declare
  v_client_id uuid;
  v_profile_id uuid;
...
begin
  ...
  where fresh_commitment_count < 200
    and from_number in (
      select phone_number
      from sms.phone_numbers
      where sending_location_id = v_sending_location_id
    ) 
  order by full_commitment_count asc
  limit 1
  into v_from_number;
