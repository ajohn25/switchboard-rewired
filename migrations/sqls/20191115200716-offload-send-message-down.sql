...
begin
  ...
  if v_prev_from_number is not null then
    ...
    insert into sms.outbound_messages (to_number, from_number, stage, sending_location_id, contact_zip_code, body, media_urls, decision_stage, estimated_segments, profile_id)
    values (send_message.to, v_prev_from_number, 'queued', v_sending_location_id, v_contact_zip_code, body, media_urls, 'prev_mapping', v_estimated_segments, v_profile_id)
    returning *
    into v_result;
  ...
  if v_from_number is not null then
    insert into sms.outbound_messages (to_number, from_number, stage, sending_location_id, contact_zip_code, body, media_urls, decision_stage, estimated_segments, profile_id)
    values (send_message.to, v_from_number, 'queued', v_sending_location_id, v_contact_zip_code, body, media_urls, 'existing_phone_number', v_estimated_segments, v_profile_id)
    returning *
    into v_result;
  ...
  if v_pending_number_request_id is not null then
    insert into sms.outbound_messages (to_number, pending_number_request_id, stage, sending_location_id, contact_zip_code, body, media_urls, decision_stage, estimated_segments, profile_id)
    values (send_message.to, v_pending_number_request_id, 'awaiting-number', v_sending_location_id, v_contact_zip_code, body, media_urls, 'existing_pending_request', v_estimated_segments, v_profile_id)
    returning *
    into v_result;
  ...
  insert into sms.outbound_messages (to_number, pending_number_request_id, stage, sending_location_id, contact_zip_code, body, media_urls, decision_stage, estimated_segments, profile_id)
  values (send_message.to, v_pending_number_request_id, 'awaiting-number', v_sending_location_id, v_contact_zip_code, body, media_urls, 'new_pending_request', v_estimated_segments, v_profile_id)
  returning *
  into v_result;
...
alter table sms.outbound_messages alter column sending_location_id set not null;
-- no easy way to drop 'processing' from sms.outbound_message_stages enum
