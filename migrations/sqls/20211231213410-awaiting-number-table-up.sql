create table sms.outbound_messages_awaiting_from_number (
  ...
  pending_number_request_id uuid not null references sms.phone_number_requests (id), 
  send_after timestamp,
  processed_at timestamp,
  decision_stage text
);

create index on sms.outbound_messages_awaiting_from_number (pending_number_request_id);

DROP FUNCTION sms.process_message;
CREATE OR REPLACE FUNCTION sms.process_message(message sms.outbound_messages, prev_mapping_validity_interval interval DEFAULT '14 days'::interval)
RETURNS json 
