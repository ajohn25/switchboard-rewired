...
begin
  ...
  if v_prev_from_number is not null then
    ...
    values (send_message.to, v_prev_from_number, 'queued', v_sending_location_id, v_contact_zip_code, body, media_urls, 'prev_mapping')
    returning *
    into v_result;
...
alter table sms.outbound_messages drop column estimated_segments;
