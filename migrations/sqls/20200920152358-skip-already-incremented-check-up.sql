alter table sms.outbound_messages add column first_from_to_pair_of_day boolean default true;
...
CREATE TRIGGER _500_increment_commitment_bucket_after_insert
  ...
  FOR EACH ROW
  WHEN (
    (new.from_number IS NOT NULL)
    AND
    (NEW.first_from_to_pair_of_day = true)
  )
...
CREATE TRIGGER _500_increment_commitment_bucket_after_update
  ...
  FOR EACH ROW
  WHEN (
    (old.from_number IS NULL) AND (new.from_number IS NOT NULL)
    AND
    (NEW.first_from_to_pair_of_day = true)
  )
  EXECUTE PROCEDURE sms.increment_commitment_bucket_if_unique();
